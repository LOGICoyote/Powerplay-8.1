...
public class Blue_AudiencesVPark extends LinearOpMode {
    ...
    public void move(double LF, double RF,double LB,double RB, MaxSonarI2CXL sensor, int tdist) {
        ...
        while ((X == 1 && sensor.getDistanceSync() > tdist) || (X == 0 && sensor.getDistanceSync() < tdist)) {
            ...
            leftFront.setPower((LF * SV) + (.05));
            rightFront.setPower((RF * SV) + (.05));
            leftBack.setPower((LB * SV) + (.05));
            rightBack.setPower((RB * SV)+ (.05));
